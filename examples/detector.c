...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, char *outfile, int fullscreen)
{
    ...
    while(1){
        if(filename){
            ...
            //config
            //FILE *f = fopen("/home/thanhtung/darknet_ttt_emu/results/input.txt","w");
            int dd=sized.h*sized.w*sized.c;
            // printf("w: %d, h: %d, c: %d, dd: %dd", sized.w, sized.h, sized.c,dd);
            // for(int i = 0;  i< sized.h*sized.w*sized.c; ++i){
            // fprintf(f,"%f\n", sized.data[i]);  
            // }
            //  fclose(f);
